...
#### 🔷 PBI-021: Automatic Daily Quests & Progress Reset Engine on New Day
* **Status:** 🟢 **DONE**
* **Priority:** Highest (P0)
* **Estimation:** 5 Story Points
* **User Story:**
  > ในฐานะ **ผู้ใช้งานจริงที่เริ่มวันใหม่ในทุกๆ เช้า**  
  > ฉันต้องการ **ให้เควสต์ประจำวันและความคืบหน้า (Progress Bar) รีเซ็ตเป็นยังไม่เสร็จ (0%) อัตโนมัติเมื่อขึ้นวันใหม่ โดยที่เหรียญที่ได้ไปแล้วและประวัติการออกกำลังกายในอดีตไม่สูญหาย**  
  > เพื่อที่ **ฉันจะได้เริ่มต้นลุยเป้าหมายใหม่ในวันนั้นได้อย่างสดชื่น มีไฟในการสะสมเหรียญต่อ และบันทึกประวัติความสม่ำเสมอได้อย่างถูกต้อง 100%**
* **Acceptance Criteria (DoD):**
  - [x] สร้างโมดูล `src/lib/dateUtils.ts` เพื่อจัดการเวลาท้องถิ่นจริง (Local Timezone เช่น UTC+7 ของประเทศไทย) แก้ปัญหาเวลาเพี้ยนจาก UTC
  - [x] เพิ่มระบบ **New Day Detection Engine** ใน Redux Store (`questSlice.ts`) ตรวจจับการขึ้นวันใหม่ (`checkAndResetForDate`)
  - [x] เมื่อขึ้นวันใหม่ ระบบจะรีเซ็ตสถานะ `completed` ของเควสต์ทั้งหมดกลับเป็น `false` ทันที ความคืบหน้าหน้าเว็บจะรีเซ็ตเป็น 0% พร้อมเริ่มต้นใหม่
  - [x] **รักษาประวัติและความคืบหน้าเดิม 100%:** ประวัติในปฏิทิน (`history[date]`), สถิติ 7 วัน, สตรีคต่อเนื่อง, ยอดเหรียญ และตั๋วรางวัล จะไม่ได้รับผลกระทบใดๆ
  - [x] **ระบบป้องกัน Server Sync ย้อนหลัง (Cloud Guard):** หากข้อมูลบนเซิร์ฟเวอร์อัปเดตครั้งล่าสุดเมื่อวาน Client จะรีเซ็ตสถานะเควสต์เป็นของวันนี้ และซิงก์กลับขึ้น Cloud ทันที ป้องกันเซิร์ฟเวอร์นำเควสต์ของเมื่อวานมาทับสถานะวันนี้
  - [x] **ระบบ Midnight Rollover Watcher (ไม่ต้องรีเฟรชหน้าเว็บ):** มี Timer ตรวจจับการก้าวข้ามเที่ยงคืน หากผู้ใช้เปิดหน้าเว็บทิ้งไว้ข้ามคืน ระบบจะรีเซ็ตเควสต์และคำคมประจำวันให้ทันทีโดยอัตโนมัติ
  - [x] เพิ่มป้ายเตือน `รีเซ็ตทุกเที่ยงคืน 🌙` และปุ่ม `รีเซ็ตวันนี้` ในส่วนหัวของรายการเควสต์ พร้อมหน้าต่างยืนยันและเสียงเอฟเฟกต์ เพื่อให้ผู้ใช้สามารถสั่งรีเซ็ตเควสต์วันนี้เองได้ทุกเมื่อที่ต้องการ
